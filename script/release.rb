...
module ReleaseTools
  ...
  def verify(root, tag = nil)
    ...
    gem_path = "pkg/appium_lib_core-#{gem_version}.gem"
    package = Gem::Package.new(File.join(root, gem_path))
    package.verify
    spec = package.spec
    raise 'Gem name or version differs from release metadata' unless spec.name == 'appium_lib_core' && spec.version == gem_version
