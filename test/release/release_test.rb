...
class ReleaseTest < Minitest::Test
  ...
  def test_existing_changelog_format
    build_gem
    assert_equal 'pkg/appium_lib_core-13.1.1.gem', ReleaseTools.verify(@root, 'v13.1.1')
  end
  ...
  def test_release_please_date_update_is_idempotent
    ...
    build_gem
    assert_equal 'pkg/appium_lib_core-13.1.1.gem', ReleaseTools.verify(@root)
  end

  def test_manifest_mismatch
    File.write(File.join(@root, '.release-please-manifest.json'), JSON.generate('.' => '13.1.0'))
    assert_raises(RuntimeError) { ReleaseTools.verify(@root) }
  end

  def test_changelog_mismatch
    File.write(File.join(@root, 'CHANGELOG.md'), "## [13.1.0] - 2026-09-12\n")
  ...
  def test_prerelease_uses_rubygems_normalized_filename
    ...
    assert_equal 'pkg/appium_lib_core-13.2.0.pre.rc.1.gem', ReleaseTools.verify(@root, 'v13.2.0-rc.1')
  end

  def test_wrong_gem_identity
    path = build_gem(name: 'different-gem')
    FileUtils.mv(path, File.join(@root, 'pkg/appium_lib_core-13.1.1.gem'))
    assert_raises(RuntimeError) { ReleaseTools.verify(@root) }
  end

  def test_wrong_packaged_version
    path = build_gem(version: '13.1.0')
    FileUtils.mv(path, File.join(@root, 'pkg/appium_lib_core-13.1.1.gem'))
    assert_raises(RuntimeError) { ReleaseTools.verify(@root) }
  end
  ...
  def build_gem(name: 'appium_lib_core', version: '13.1.1', include_version: true)
