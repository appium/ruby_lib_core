...
3. Create the GitHub Actions environment `rubygems`. Permit release tags (`v*`) and
   the `master` branch (for manual retry runs). Add a required reviewer only if a
   separate approval for each publication is wanted; otherwise merging the release
   PR is the normal human decision.
4. As an owner of `appium_lib_core` on RubyGems.org, configure a Trusted Publisher:
...
2. Release Please maintains a PR updating `lib/appium_lib_core/version.rb`,
...
`release_notes.md` is retained as historical commit-level notes through 13.1.0.
